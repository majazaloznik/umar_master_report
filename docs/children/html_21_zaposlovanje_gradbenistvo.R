...
fig1 <- data |>
  ...
  fig2 <- data2 |>
  plot_ly(x = ~period, width = 1000, height = 600) |>
  add_lines(y = ~`raw`,  hovertemplate="%{x|%b-%Y} %{y:.2f}%",#showlegend = FALSE,
            name = "Nezglajeni podatki", color = I(umar_cols()[3])) |>
  add_lines(y = ~`value`,  hovertemplate="%{x|%b-%Y} %{y:.2f}%",#showlegend = FALSE,
            name = "Pri\u010dakovano zaposlovanje v gradbeni\u0161tvu", color = I(umar_cols()[5])) |>
  layout(annotations = list(x = 0 , y = 1, showarrow = F,
...
subplot( fig1, fig2, nrows = 2, shareX = TRUE) |>
  rangeslider(as.Date("2006-01-01"), max(data$period)+10) |>
  layout(font=list(family = "Myriad Pro"),
         autosize = F, margin = m,
         yaxis = list(title = list(text="Medletna rast, v %",
                                   font = list(size =12))),
         yaxis2 = list(title = list(text="Ravnote\u017eje",
                                    font = list(size =12))),
         xaxis = list(title = "",
