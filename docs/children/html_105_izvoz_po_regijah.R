# get data
df <- read.csv2(here::here("data/086.csv"), encoding = "UTF-8")
...
fig1 <- plot_ly(data, x = ~period, width = 1000,
                height = 600) |>
  add_lines_m(y = ~value.x,  name = "Izvoz blaga v EU",  color = I(umar_cols()[1])) |>
  add_lines_m(y = ~value.y,  name = "Izvoz blaga izven EU *",  color = I(umar_cols()[2])) |>
  add_lines_m(y = ~value,  name = "Izvoz blaga *",  color = I(umar_cols()[3])) |>
...
subplot(fig1, fig2,  nrows = 2, shareX = TRUE) |>
  umar_layout(slider_w, m,
              ...
              yaxis2 = umar_yaxis('Indeks (povprečje 2010)'),
              xaxis = umar_xaxis("M"),
