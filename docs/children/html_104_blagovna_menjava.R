# get data
df <- read.csv2(here::here("data/085.csv"), encoding = "UTF-8")
...
fig1 <- plot_ly(data, x = ~period, width = 1000,
                height = 600) |>
  add_lines_m(y = ~value.x,  name = "Izvoz blaga",  color = I(umar_cols()[1])) |>
  add_lines_m(y = ~value.y,  name = "Uvoz blaga",  color = I(umar_cols()[2])) |>
