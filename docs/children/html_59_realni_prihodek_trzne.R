...
data |>
  ...
  add_lines(y = ~`value.x`, name = "Tr\u017ene storitve (H+I+J+L+M+N)",  color = I(umar_cols()[1])) |>
  add_lines(y = ~`value.y`, name = "Promet in skladi\u0161\u010denje (H)",  color = I(umar_cols()[2])) |>
  add_lines(y = ~`value.x.x`, name = "Gostinstvo (I)",  color = I(umar_cols()[3])) |>
