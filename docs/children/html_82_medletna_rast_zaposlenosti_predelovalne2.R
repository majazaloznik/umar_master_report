...
fig1 <- plot_ly(data, x = ~period, width = 1000,
  ...
  add_lines_mp(y = ~value.x,  name = "Gumarska industrija",  color = I(umar_cols()[1])) |>
  add_lines_mp(y = ~value.y,  name = "Proizvodnja nekovin. mineral. izd.",  color = I(umar_cols()[2])) |>
  add_lines_mp(y = ~value.x.x,  name = "Proizvodnja kovin",  color = I(umar_cols()[3])) |>
  ...
  my_panel_subtitle("Rast zaposlenosti v srednje nizko tehnolo\u0161ko zahtevnih dejavnostih")
...
fig2 <- plot_ly(data2, x = ~period, width = 1000,
                height = 800) |>
  add_lines_mp(y = ~skupaj,  name = "Predelovanle dejavnosti skupaj",  color = I(umar_cols()[3]), fill = "tozeroy") |>
  add_lines_mp(y = ~value.x,  name = "Kemi\u010d industrija",  color = I(umar_cols()[6])) |>
  add_lines_mp(y = ~value.y,  name = "Proiz. el. naprav",  color = I(umar_cols()[7])) |>
  add_lines_mp(y = ~value.x.x,  name = "Proiz. dr. strojev in naprav",  color = I(umar_cols()[8])) |>
  ...
  my_panel_subtitle("Rast zaposlenosti v srednje visoko tehnolo\u0161ko zahtevnih dejavnostih")
...
fig3 <- plot_ly(data3, x = ~period, width = 1000,
  ...
  my_panel_subtitle("Rast zaposlenosti v visoko tehnolo\u0161ko zahtevnih dejavnostih")
...
subplot(fig1, fig2, fig3,  nrows = 3, shareX = TRUE) |>
  umar_layout(slider_w, m,
    ...
    xaxis = umar_xaxis("M"),
    title = umar_subtitle(updated, "UMAR", prep_l0$transf_txt),
