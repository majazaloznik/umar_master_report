...
prep_l2 <- prep_multi_line(spl[[14]], con)
...
plot_ly(data2, x = ~period, hovertemplate="%{x|%b-%Y} %{y:.2f}", width = 1000) |>
  add_lines(y = ~value.x,  name = "Kazalnik zaupanja v trgovini na drobno",  color = I(umar_cols()[1]) ) |>
  add_lines(y = ~value.y,   name = "Prodaja",color = I(umar_cols()[2])) |>
  add_lines(y = ~value.x.x,  name = "Pri\u010dakovana prodaja",  color = I(umar_cols()[3])) |>
  add_lines(y = ~value.y.y,   name = "Obseg zalog",color = I(umar_cols()[4])) |>
