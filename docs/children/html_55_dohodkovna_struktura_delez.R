...
data |>
  ...
  add_bars(y = ~`value.x`, name = "Bruto poslovni prese\u017eek in razn. doh.",  color = I(umar_cols()[2])) |>
  add_bars(y = ~`value.y`, name = "Sredstva za zaposlene",  color = I(umar_cols()[3])) |>
  add_bars(y = ~`value.x.x`, name = "Davki na proizvodnjo in uvoz",  color = I(umar_cols()[4])) |>
  add_bars(y = ~-`value.y.y`, name = "Subvencije na proizvodnjo",  color = I(umar_cols()[1])) |>
