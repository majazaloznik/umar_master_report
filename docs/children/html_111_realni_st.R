...
plot_ly(data, x = ~period, width = 1000) |>
  add_lines_mp(y = ~value.x,  name = "Strokovno-tehnične dejavnosti (M)",  color = I(umar_cols()[3]),
               fill = "tozeroy") |>
  add_lines_mp(y = ~value.y,  name = "Pravno-računovodske dejavnosti (M 69)",  color = I(umar_cols()[1])) |>
  add_lines_mp(y = ~value.x.x,  name = "Svetovalne storitve (M 70.2)",  color = I(umar_cols()[2])) |>
