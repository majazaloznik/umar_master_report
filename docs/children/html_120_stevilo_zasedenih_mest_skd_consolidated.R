...
# get data
df <- read.csv2(here::here("data/100.csv"), encoding = "UTF-8")
...
purrr::reduce(prep_l$data_points, dplyr::full_join, by = c("period_id", "period")) %>%
  ...
  dplyr::mutate(ostalo = sum(value.x, value.x.x, value.y.y, value.x.x.x.x.x,
                             ...
                             value.y.y.y.y.y.y.y.y.y, na.rm = TRUE), .keep = "unused") -> data_zasedena


plot_ly(data_zasedena, x = ~period, width = 1000) |>
  ...
  umar_layout(slider_w, m,
              yaxis = umar_yaxis('Število zasedenih delovnih mest'),
              xaxis = umar_xaxis("M"),
